        ...
        const observer = new IntersectionObserver(
            (entries) => {
                if (entries[0].isIntersecting) {
                    observer.disconnect();
                    // Use requestIdleCallback if available, or a small timeout to ensure it doesn't block critical tasks
                    if ('requestIdleCallback' in window) {
                        window.requestIdleCallback(() => loadScriptAndInit());
                    } else {
                        setTimeout(loadScriptAndInit, 1);
                    }
                }
            },
            { rootMargin: '0px' }
